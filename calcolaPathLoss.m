...
if ~isfield(params, 'environment_type')
    ...
    % Calcola l'attenuazione per ostacoli UNA SOLA VOLTA per il punto di misura
attenuazione_ostacoli = 0;
if isfield(params, 'numOstacoli')
    attenuazione_ostacoli = params.numOstacoli * 1; % 10 dB per ostacolo
    if isfield(params, 'altezzaEdificio') && params.altezzaEdificio > altezza_punto_misura
        attenuazione_ostacoli = attenuazione_ostacoli + calcolaDiffrazioneKnifeEdge(params.altezzaEdificio, altezza_punto_misura, distanza);
    end
end
    ...
    switch modello
        ...
        case 'COST 231-Hata + Attenuazione Ostacoli'
            path_loss_dB = calcolaCOST231Hata(params, frequenza, distanza, altezza_antenna, altezza_punto_misura) + attenuazione_ostacoli; % Passa altezza_punto_misura
...
L_urbano = 46.3 + 33.9 * log10(f_MHz) - 13.82 * log10(hb_m) - a_hm + ...
    ...
    if d_km < 1
        path_loss_dB = path_loss_dB - 78; % Riduce il path loss per distanze inferiori a 1 km
    end
